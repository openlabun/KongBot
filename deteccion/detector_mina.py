"""
detector_mina.py — Detección de la entrada a la mina en Banana Kong
Resolución BlueStacks: 960x540

Estrategia: Template matching con alpha.
La mina aparece como una entrada de madera apilada en el suelo.
"""
...
class DetectorMina:
    def __init__(self):
        ...
        ruta = os.path.join(TEMPLATES_DIR, "mina-bg.png")
        ...
        print(f"✅ Template mina: {self.template.shape[1]}x{self.template.shape[0]}px")
    ...
    def detectar_mina(self, frame):
        ...
        if mejor_val < UMBRAL or mejor_loc is None:
            cv2.putText(frame_r, "Mina: NO", (10, 60),
        ...
        cv2.rectangle(frame_r, (x_real, y_real),
                      (x_real+bw_real, y_real+bh_real), (0, 165, 255), 2)
        cv2.putText(frame_r, f"MINA {mejor_val:.2f}", (x_real, y_real-5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 1)
        return True, (cx, cy), rect, frame_r

    def probar(self):
        print("=== DETECTOR DE MINA ===  q=salir | s=guardar")
        time.sleep(2)
        cv2.namedWindow("Mina Detector")
        while True:
            frame = self.capturar_pantalla()
            if frame is None:
                time.sleep(1)
                continue
            hay_mina, pos, rect, frame_r = self.detectar_mina(frame)
            if hay_mina:
                print(f"⛏️  Mina detectada en cx={pos[0]:.2f} cy={pos[1]:.2f}")
            cv2.imshow("Mina Detector", frame_r)
            cv2.moveWindow("Mina Detector", 100, 100)
            key = cv2.waitKey(1) & 0xFF
            if key == ord("q"):
                break
            elif key == ord("s"):
                cv2.imwrite("mina_frame.png", frame_r)
                print("Guardado")
        cv2.destroyAllWindows()


if __name__ == "__main__":
    detector = DetectorMina()
    detector.probar()
