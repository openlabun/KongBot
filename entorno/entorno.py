"""
entorno.py — Entorno Gymnasium optimizado para Banana Kong RL
Resolución BlueStacks: 960x540

Observación (23 valores normalizados [0,1]):
    [0]  kong_cx
    [1]  kong_cy
    [2]  banana1_dx
    [3]  banana1_cy
    [4]  banana2_dx
    [5]  banana2_cy
    [6]  hay_agua
    [7]  hay_barril
    [8]  barril_dx
    [9]  barril_cy
    [10] hay_roca
    [11] roca_dx
    [12] roca_cy
    [13] hay_muro
    [14] muro_dx
    [15] muro_cy
    [16] hay_mina
    [17] mina_dx
    [18] mina_cy
    [19] hay_tubo
    [20] tubo_dx
    [21] tubo_cy
    [22] kong_visible

Acciones (Discrete 4):
    0 - NADA
    1 - PLANEAR
    2 - DASH
    3 - BAJAR

Recompensas:
    +0.04 por sobrevivir cada step
    +1.00 por banana recogida
    -0.02 por DASH
    -0.005 por PLANEAR/BAJAR
    -0.001 por NADA
    -0.5 por DASH cerca de mina/tubo
    -10.00 por game over
"""

import gymnasium as gym
from gymnasium import spaces
import numpy as np
import time
import cv2
import pyautogui
import os

from entorno.perceptor  import Perceptor
from controles.acciones import ModuloAcciones, NADA, PLANEAR, DASH, BAJAR

MAX_STEPS      = 2000
DELAY_ACCION   = 0.05
DELAY_REINICIO = 3.0
OBS_SIZE       = 23


class BananaKongEnv(gym.Env):
    metadata = {"render_modes": ["human"]}

    def __init__(self, render_mode=None):
        super().__init__()

        self.render_mode = render_mode

        self.observation_space = spaces.Box(
            low=0.0, high=1.0, shape=(OBS_SIZE,), dtype=np.float32
        )
        self.action_space = spaces.Discrete(4)

        self.perceptor = Perceptor()
        self.acciones  = ModuloAcciones()
        #self.perceptor.start_display()

        self._step_count      = 0
        self._total_bananas   = 0
        self._reward_episodio = 0.0
        self.perceptor.reset_colisiones()
        self._primer_episodio = True

    # ── Reset ───────────────────────────────────────────────
    def reset(self, seed=None, options=None):
        super().reset(seed=seed)
        pyautogui.keyUp('w')

        if self._primer_episodio:
            self._primer_episodio = False
        else:
            self._reiniciar_juego()

        self._step_count      = 0
        self._total_bananas   = 0
        self._reward_episodio = 0.0
        self.perceptor.reset_colisiones()

        # Esperar a que el perceptor tenga frame válido
        for _ in range(10):
            estado = self.perceptor.get_estado()
            if estado["frame"] is not None:
                break
            time.sleep(0.1)

        obs = self._estado_a_obs(estado)
        return obs, {}

    # ── Step ────────────────────────────────────────────────
    def step(self, accion):
        self.acciones.ejecutar(accion)
        time.sleep(DELAY_ACCION)

        estado = self.perceptor.get_estado()
        obs    = self._estado_a_obs(estado)

        bananas_recogidas = self.perceptor.pop_bananas_recogidas()

        # ── Reward ──────────────────────────────────────────
        reward = 0.05

        # Coste por acción
        if accion == DASH:
            reward -= 0.02
        elif accion in (PLANEAR, BAJAR):
            reward -= 0.005
        else:
            reward -= 0.001

        # Penalización contextual
        if accion == DASH and (estado.get("mina") or estado.get("tubo")):
            reward -= 0.5

        # Bananas
        reward += bananas_recogidas * 1.0

        # Game over
        terminated = False
        if estado["game_over"] and self._step_count > 10:
            reward -= 10.0
            terminated = True

        self._reward_episodio += reward
        self._total_bananas   += bananas_recogidas
        self._step_count      += 1

        self.perceptor._total_bananas = self._total_bananas
        self.perceptor._step_count    = self._step_count

        truncated = self._step_count >= MAX_STEPS

        if terminated or truncated:
            print(f"[ep end] steps={self._step_count} bananas={self._total_bananas} reward={self._reward_episodio:+.2f}")

        return obs, reward, terminated, truncated, {}

    # ── Observación ─────────────────────────────────────────
    def _estado_a_obs(self, estado):
        obs = np.zeros(OBS_SIZE, dtype=np.float32)

        # Kong
        if estado["kong"]:
            kong_cx, kong_cy = estado["kong"]
            obs[0] = kong_cx
            obs[1] = kong_cy
            obs[22] = 1.0
        else:
            kong_cx, kong_cy = 0.3, 0.5  # solo para ordenar
            obs[0] = 0.3
            obs[1] = 0.5
            obs[22] = 0.0

        # Bananas
        bananas = sorted(
            estado["bananas"]["posiciones"],
            key=lambda p: abs(p[0] - kong_cx)
        )
        for i, (cx, cy) in enumerate(bananas[:2]):
            obs[2 + i*2] = np.clip(cx - kong_cx + 0.5, 0.0, 1.0)
            obs[3 + i*2] = cy

        # Agua
        obs[6] = 1.0 if estado.get("agua") else 0.0

        # Barriles
        barriles = estado.get("barriles", [])
        if barriles:
            cercano = min(barriles, key=lambda p: abs(p[0] - kong_cx))
            obs[7] = 1.0
            obs[8] = np.clip(cercano[0] - kong_cx + 0.5, 0.0, 1.0)
            obs[9] = cercano[1]
        else:
            obs[7] = 0.0
            obs[8] = 0.5
            obs[9] = 0.5

        # Rocas
        rocas = estado.get("rocas", [])
        if rocas:
            cercano = min(rocas, key=lambda p: abs(p[0] - kong_cx))
            obs[10] = 1.0
            obs[11] = np.clip(cercano[0] - kong_cx + 0.5, 0.0, 1.0)
            obs[12] = cercano[1]
        else:
            obs[10] = 0.0
            obs[11] = 0.5
            obs[12] = 0.5

        # Muros
        muros_pos = []
        for mx, my, mw, mh in estado.get("muros_rects", []):
            cx_m = (mx + mw/2) / 960
            cy_m = (my + mh/2) / 540
            muros_pos.append((cx_m, cy_m))

        if muros_pos:
            cercano = min(muros_pos, key=lambda p: abs(p[0] - kong_cx))
            obs[13] = 1.0
            obs[14] = np.clip(cercano[0] - kong_cx + 0.5, 0.0, 1.0)
            obs[15] = cercano[1]
        else:
            obs[13] = 0.0
            obs[14] = 0.5
            obs[15] = 0.5

        # Mina
        if estado.get("mina_pos"):
            obs[16] = 1.0
            obs[17] = np.clip(estado["mina_pos"][0] - kong_cx + 0.5, 0.0, 1.0)
            obs[18] = estado["mina_pos"][1]
        else:
            obs[16] = 0.0
            obs[17] = 0.5
            obs[18] = 0.5

        # Tubo
        if estado.get("tubo_pos"):
            obs[19] = 1.0
            obs[20] = np.clip(estado["tubo_pos"][0] - kong_cx + 0.5, 0.0, 1.0)
            obs[21] = estado["tubo_pos"][1]
        else:
            obs[19] = 0.0
            obs[20] = 0.5
            obs[21] = 0.5

        return obs

    # ── Render ────────────────────────────────────────────────────────
    def render(self):
        pass  # El display corre en hilo propio del perceptor

    # ── Reinicio del juego ────────────────────────────────────────────
    def _reiniciar_juego(self):
        tpl_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                               "deteccion", "templates")
        tpl_flecha     = self._cargar_template(os.path.join(tpl_dir, "flecha.png"))
        tpl_play_again = self._cargar_template(os.path.join(tpl_dir, "play_again.png"))

        print("🔄 Reiniciando juego...")
        time.sleep(5.5)

        if not self._esperar_y_clicar(tpl_flecha, timeout=9.0, etiqueta="flecha"):
            print("  ⚠️ Flecha no encontrada — presionando W")
            pyautogui.press('w')
        time.sleep(2.0)

        if not self._esperar_y_clicar(tpl_play_again, timeout=9.0, etiqueta="Play Again"):
            print("  ⚠️ Play Again no encontrado — presionando W")
            pyautogui.press('w')
        time.sleep(1.0)

        self.perceptor.det_kong.reset()
        time.sleep(DELAY_REINICIO)
        print("✅ Juego reiniciado")

    def _cargar_template(self, ruta):
        img = cv2.imread(ruta, cv2.IMREAD_UNCHANGED)
        if img is None:
            return None
        if img.shape[2] == 4:
            return cv2.cvtColor(img[:,:,:3], cv2.COLOR_BGR2GRAY), img[:,:,3]
        return cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), None

    def _esperar_y_clicar(self, template, timeout=8.0, etiqueta=""):
        if template is None:
            return False
        t_gris, t_alpha = template
        inicio = time.time()

        while time.time() - inicio < timeout:
            estado = self.perceptor.get_estado()
            frame  = estado["frame"]
            if frame is None:
                time.sleep(0.2)
                continue

            gris = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

            for escala in [0.9, 1.0, 1.1]:
                h_t, w_t = t_gris.shape
                nw, nh = int(w_t*escala), int(h_t*escala)
                if nw >= gris.shape[1] or nh >= gris.shape[0]:
                    continue

                t_s = cv2.resize(t_gris, (nw, nh))
                if t_alpha is not None:
                    a_s = cv2.resize(t_alpha, (nw, nh))
                    res = cv2.matchTemplate(gris, t_s, cv2.TM_CCOEFF_NORMED, mask=a_s)
                else:
                    res = cv2.matchTemplate(gris, t_s, cv2.TM_CCOEFF_NORMED)

                _, val, _, loc = cv2.minMaxLoc(res)
                if val >= 0.65:
                    cx = loc[0] + nw//2 + self.perceptor.ventana.left
                    cy = loc[1] + nh//2 + self.perceptor.ventana.top
                    print(f"  ✓ {etiqueta} (conf={val:.2f})")
                    pyautogui.click(cx, cy)
                    return True
            time.sleep(0.3)
        return False

    def close(self):
        pyautogui.keyUp('w')
        cv2.destroyAllWindows()
        print("👋 Entorno cerrado")