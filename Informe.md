...
**Detectores de visión por computador:** Cada tipo de objeto tiene su propio módulo en `deteccion/`. La estrategia es híbrida HSV + template matching para la mayoría, excepto el game over (template matching puro sobre ROI pequeño) y el agua (HSV puro). Los templates son PNG con canal alpha real usado como máscara en `cv2.matchTemplate(..., mask=alpha)`. La mayoría de detectores usa `TM_CCOEFF_NORMED` por ser robusto ante variaciones de brillo; el detector de Kong usa `TM_SQDIFF_NORMED` (conversión a similitud mediante `1 - min_val`) porque mostró mejor discriminación con sus templates específicos. Los detectores de rocas y mina trabajan a mitad de resolución (`FACTOR_RESIZE=0.5`) para reducir el tiempo de procesamiento en ~4x. Durante el desarrollo se realizaron sesiones de calibración sistemática registrando área, ratio, solidez y confianza de cada detección para ajustar umbrales.
...
**Entorno Gymnasium:** El vector de observación contiene 23 floats normalizados en [0,1]. Para cada obstáculo se incluye un flag de presencia (0/1) más coordenadas relativas a Kong centradas en 0.5, resolviendo la ambigüedad de usar 0.0 como valor por defecto (que el modelo podría interpretar como "obstáculo en la esquina"). La recompensa base es +0.05 por step de supervivencia, más +1.0 por banana recogida. Cada acción tiene un costo: -0.02 por DASH, -0.005 por PLANEAR/BAJAR, -0.001 por NADA. Además, ejecutar DASH cerca de una mina o tubo tiene una penalización contextual de -0.5 para desalentar dashes peligrosos. La penalización por game over es -10.0. El reinicio automático usa template matching para las pantallas de "Revive" y "Play Again", con fallback de tecla W.
